...
4. Click **Save As** to choose a folder and filename for the estimate, its complete pricing library and all three calculators. **Save** updates the currently opened or saved project file without another file dialog. If no project file is open, use **Save As** first. Reopen a file through **Projects** or the native **Load Project** dialog; use **Use current pricing** when you want to replace its original prices with the last saved shared library. A file changed or removed outside the app must be reopened or saved through **Save As** before it can be overwritten.
5. In **Libraries → Pricing Library**, choose **Shared library** or **Current project pricing**. Each product has one row with an editable **Product/Service** label, supplier price, markup, one **Sell price** column, two Estimator group boxes and one yield. Existing selection keys and saved per-use rate overrides remain intact. Retained legacy estimator rates appear as a read-only notice beneath **Sell price**. Yield units are read-only and follow the calculation. **Estimator availability** lets you edit both Main Estimator and Firestopping Estimator groups as semicolon-separated lists. Filter by either estimator, or search for an item or group; products unused by both estimators and standalone rates remain available. **Save pricing** stores shared-library edits, while **Apply project pricing** updates only the active estimate. **Save / Save As** stores its project prices in the project file.
...
The persistent **Current project** area shows the known location and last-saved time. Save state appears directly under the main logo, including when any estimate, project-pricing or calculator input has unsaved changes. Files opened through **Load Project** or **Projects** retain their save target for this app session. Imported browser-upload content has no authorized local path and requires **Save As**. If the app server restarts, reopen the file or use **Save As** before saving again. On Windows, native Save As, Load Project and folder dialogs are owned by the foreground application and the actual dialog, including overwrite prompts, is raised above it. Automated checks cover save/load behavior and native helper structure; native dialog foreground placement remains a manual visual check.
...
New steel-spray estimates start with blank direct yields and these bag-mass/estimating-density pairs (kg/bag, kg/m³): CAFCO 300 (20, 308), MANDOLITE CP2 (20, 388), FENDOLITE MII (20, 645), PERLIFOC HP ECO+ (17, 410), MONOKOTE MK-6 HY (21.8, 344), and MONOKOTE Z106 (22.2, 325). Yield used is calculated as bag mass divided by estimating density until a verified direct yield is entered. These are user-selected estimating inputs, not published installed densities or confirmed site yields. Existing saves retain their exact settings. Numeric material settings remain adjustable; **Material basis / reference** is read-only. Projects saved with the previous reviewed reference wording remain loadable and keep that wording. Z106 has its own Settings tab and bag quantities, and shares MK-6 HY technical thickness/fire-resistance lookups. The packaged source workbook is unchanged.

Ductwork PRODUCT SETTINGS also has editable CAFCO and MONOKOTE MK-6 bag mass and estimating density. **Uncalibrated** working net yield uses bag mass divided by estimating density for each product; **Calibrated** retains its existing reference calculation, and the injection choice still applies its existing multiplier. The informational location/example text beside the penetration takeoff table is hidden in the browser; the technical takeoff table and source content remain available.
...
The calculator toolbar places **Download XLSX Schedule** before **Download PDF Schedule**, followed by **Download PDF Summary**. **Export Template**, **Reset Calc** and **Recalculate** use neutral styling in the calculator heading. Editable schedule Exposure values use normal-weight text; headings and technical reference labels keep their emphasis.
...
inputs for import, continue using **Export Template** and **Import XLSX Schedule**.
...
- `data/vermiculite_yield_defaults.json`: user-selected startup estimating inputs with historical evidence links, separate from the original source graph and existing saves.
...
1. Click **Export Excel** in **Pricing library**. The `.xlsx` includes the entire effective library, including unsaved edits and items excluded by the current filter, with **Inventory & Rates** and **Instructions** sheets.
