...
Requires Python 3.11 or newer. There are **no third-party runtime dependencies** and neither Excel nor the workbooks are required to run the app.
...
Node is only needed for the JavaScript syntax check. The build creates `dist/ceasefire-estimator.zip`, containing the standalone application, imported data and README. Extract it and run `python -m estimator` in that directory.
