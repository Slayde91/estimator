...
5. In **Pricing library**, use the single **Inventory & Rates** view to edit supplier prices, markup and manual service prices. Expand a product's **Used in** section to edit each category's selection rate and yield. Filter by **Used in Estimator** or search for an item or use. Product pricing appears once even when the product has several uses; unused items and standalone rates remain available. The product's **Reset row** restores its imported values; **Reset rate** and **Reset yield** act separately within its uses. Use **Save pricing** to apply your changes.
...
Enter inputs directly, or click **Export XLSX Template**, fill its schedule in Excel and use **Import XLSX Schedule**. Import replaces the complete schedule, including clearing unused old rows, and preserves other calculator settings. It stays a draft until **Save calculator**. Each calculator keeps its own saved inputs and settings. **Reset Calc** restores the example schedule and default settings as a draft; vermiculite uses the reviewed material defaults described below. **Recalculate** appears immediately before **Save calculator** in the calculator heading.
...
The templates contain 1,000 prepared rows, an informational Line column and reference instructions. Duct has eight editable fields, board 24 and spray 13 including Location. Spray templates show Location second; board retains its existing input order. Original templates with their exact legacy headings remain importable. Import accepts the exported values-only `.xlsx` layout, up to 5 MB; it does not map an arbitrary schedule layout or execute uploaded formulas. The [schedule extension mapping](docs/SCHEDULE_EXTENSION.md) explains the application ranges and preserved source coordinates.
...
5. Click the green **Import Excel** button, select the complete workbook, and review the added, removed and updated rows. Apply it to the draft, then click **Save pricing** to update the active library and dropdown choices. Import alone does not save. **Discard changes** keeps the previously saved library. Earlier combined workbooks with separate **Inventory** and **Use** rows, and older exports with separate **Inventory** and **Rates** sheets, are still accepted.
