...
5. In **Pricing library**, use the single **Inventory & Rates** view to edit supplier prices, markup and manual service prices. Filter by **Used in Estimator**, search for an item, and expand **Used in** to edit its category rates and yields. Items with no uses and standalone rates remain available. **Reset row** restores imported product values; the use controls can reset a price or yield separately. Use **Save pricing** to apply your changes.
...
2. Edit **Inventory** rows for product details, supplier prices, markup and selling prices. Expand the outline beneath a product to edit its **Use** rows: category, selection name, rate and yield. Each product's purchasing values appear once. Standalone uses without an inventory link are included too.
