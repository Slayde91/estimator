...
5. In **Pricing library**, use the single **Inventory & Rates** view to edit supplier prices, markup and manual service prices. Each category, selection name, sell rate and yield is visible beside its product. Filter by **Used in Estimator** or search for an item. Product pricing appears once even when the product has several uses; unused items and standalone rates remain available. **Reset product** restores imported product values; **Reset rate** and **Reset yield** act separately. Use **Save pricing** to apply your changes.
...
2. Edit **Inventory** rows for product details, supplier prices, markup and selling prices. All **Use** rows are visible beneath their linked products and contain Group, selection name, Price source, rate, Yield type, Yield, Rate ID and Use order. Those use-only fields are deliberately blank on Inventory rows. Each product's purchasing values appear once. Standalone uses without an inventory link are included too; optional product properties remain in expandable columns P:Z.
3. Add rows or delete products and choices. Preserve existing IDs. Blank IDs receive new IDs; to link a new product and use in the same file, enter your own matching unique **Inventory ID** on both rows. Links follow IDs, even after sorting. **Use order** preserves the order within each dropdown. Remove or unlink uses pointing to a deleted product. Keep the provided headers and supported group keys.
4. For linked rates, **Price source = Inventory** follows the inventory sell price. Editing a use's **Sell price / rate** makes it an **Override**; choose **Inventory** to restore the link. Supplier markup items calculate their sell price when supplier price or markup changes. Manual items accept a sell price directly.
5. Click the green **Import Excel** button, select the complete workbook, and review the added, removed and updated rows. Apply it to the draft, then click **Save pricing** to update the active library and dropdown choices. Import alone does not save. **Discard changes** keeps the previously saved library. Older exports with separate **Inventory** and **Rates** sheets are still accepted.

Each workbook replaces the inventory and uses in full, including collapsed, hidden or filtered rows. The Instructions sheet explains markup, supported rate groups and the separate Number/Blank/Empty text/Not used yield types. Blank and empty-text yields have different estimating behavior and are preserved. Exported strings are literal text; import accepts values-only `.xlsx`, with no formulas, macros, external links or extra data sheets. The limit is 5 MB, 5,000 inventory rows and 5,000 use rows, with bounded ZIP contents and validated finite numbers. Use the exported template; arbitrary supplier spreadsheet layouts are not mapped automatically. The original `Inventory_list.xlsm` remains the immutable initial source, not the runtime import template.
