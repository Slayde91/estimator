...
Reviewed 14 September 2026. This records the historical published-yield evidence and native Excel comparison for the former direct-yield startup profile. On 25 September 2026, the user selected new startup estimating densities, blank direct yields and a separate MONOKOTE Z106 bag profile. The current inputs are in `data/vermiculite_yield_defaults.json` and described in README; the historical scenarios below remain regression evidence, not the active default profile. Existing saved inputs and the packaged source workbook remain separate. This is not fire-protection design approval.

The Z106 application extension reuses the MK-6 HY open-section thickness tables covered by [BRANZ FAR4853 Issue 3](https://docadmin.tfire.com.au/exfiles/FAR4853%20ISSUE%203%20MONOKOTE%20MK6%20Z106%20REPORT%20FINAL%20-%20Jan%202021.pdf) and the hollow-section tables covered by [BRANZ FAR4856 Issue 2](https://tfire.com.au/documents/Internal-Fire-Hollow-Steel-Sections-FAR4856). Each report names MK-6/HY, MK-6s and Z-106 together for the listed cases and periods. The application's existing MK-6 case limits and review messages also apply to Z106. Z106 bag mass is 22.2 kg per the [GCP product data](https://gcpat.com.au/en-gb/solutions/products/monokote-fireproofing/monokote-z-106hy); the 325 kg/m³ startup estimating density was supplied by the user and is not the manufacturer's minimum installed density.

## Recommendation

The historical profile replaced CAFCO 300's inherited estimating assumption with its published Australian theoretical coverage. It retained the other four products' existing coverage basis with clearer qualifications and used the current Australian MONOKOTE bag mass. These were starting estimates before site losses, not guaranteed site yields.
...
At the September 14 review, the document registry described CAFCO and CP2 SDS contents as December 2022 while the stable URLs returned April 2026 content. The registry dates need separate reconciliation. Read document contents and dates, not URL filenames.
...
At the September 14 checkpoint, all 6,565 selected outputs matched the application engine, and the then-installed default profile matched 863 numeric and quantity-status native Excel outputs. Numeric comparison used absolute tolerance `1e-10` and relative tolerance `1e-12`; strings, errors, booleans and blanks used exact comparisons. The retained scenarios still test source calculation parity. The current September 25 startup defaults differ from those captured historical inputs, so the historical fixture is not claimed as a direct oracle for the new defaults or Z106.
