...
function argumentsFrom(argv) {
  const options = { workers: 2, limit: Infinity };
  ...
  if (!options.directory || !options.output || !Number.isInteger(options.workers)
    ...
    throw new Error('Usage: node ocr_trafalgar_documents.cjs PREPARED_DIRECTORY OUTPUT_JSON [--workers 1..4] [--limit N] [--lang-path DIRECTORY] [--rotation-map JSON_FILE]');
  }
  return options;
}
...
async function main() {
  ...
  const save = () => {
    const result = { schema_version: 1, index_sha256: indexSha256,
      ...
      counts: { eligible_unique_images: groups.size, completed_images: completed.size,
