...
(async()=>{
  ...
  await check('Firestopping summary stays catalog-wide while the exact reference filter combines with search and other filters',async h=>{
    h.setRoute(path=>path==='/api/libraries'?{libraries:[{id:'penetration',available:true,count:100,unlinked_count:24}]}:({...records('penetration'),counts:{total:100,linked:76,unlinked:24},filters:[...filters,{key:'technical_reference',label:'Server label',options:['any','linked','unlinked']}]}));
    await h.api.open('penetration');const pane=h.pane('penetration');assert.deepEqual(walk(pane.summary).filter(node=>node.tagName==='td').map(node=>node.textContent),['100','24']);assert.match(pane.count.textContent,/of 1 records/);
    ...
    for(const value of ['linked','unlinked','any']){const control=walk(pane.filterControls).find(node=>node.dataset.libraryFilter==='technical_reference');control.value=value;await control.emit('change');await flush();assert.match(h.calls.at(-1).path,new RegExp('technical_reference='+value));assert.match(h.calls.at(-1).path,/search=kept\+search.*category=sample\+%26\+test/);assert.deepEqual(walk(pane.summary).filter(node=>node.tagName==='td').map(node=>node.textContent),['100','24']);}
  });
  ...
  await check('Link failures retain selection for retry, reject unconfirmed receipts, and duplicate receipts are successful',async h=>{
    ...
    await h.api.open('penetration');const pane=h.pane('penetration');await walk(pane.results).find(node=>node.dataset.libraryLink).emit('click');await flush();const session=pane.linkSession;await walk(session.results).find(node=>node.dataset.libraryLinkChoice).emit('change');await session.save.emit('click');assert.equal(pane.linkSession,session);assert.equal(session.selected,'technical-1');assert.equal(session.save.disabled,false);assert.match(session.message.textContent,/not saved.*Target is unavailable/);
