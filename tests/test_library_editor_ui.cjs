...
(async()=>{
  await check('Library editor shows only the pipe category and Bulkhead relevant to the item',async h=>{
    ...
    assert.match(text(h.byId('library-editor-groups')),/Penetration.*Cables\/Bundles.*Bulkhead.*OTHER/);assert.equal(h.byId('library-editor-groups').children.at(-1).textContent,'OTHER');assert.doesNotMatch(text(h.byId('library-editor-groups')),/Unlagged Pipes|Plastic Pipes/);
  ...
  await check('Library automatic allowances remain raw and clean until manual input, retaining precise and zero overrides and reset',async h=>{
    const metadata=allowanceDefinition(),source=fixture(metadata.defaults,{definition:metadata,result:allowanceResult(metadata.defaults,metadata)});h.api.present(source);h.audit.state.group='Pipes';h.audit.renderFields();let control=h.control('pipe_labour_hours');const before=copy(h.audit.state.draft);
    assert.equal(control.value,'0.30');assert.equal(h.api.hasUnsavedChanges(),false);await control.focus();await control.blur();assert.deepEqual(copy(h.audit.state.draft),before);
    ...
    for(const value of [0,.123456789012345]){control=h.control('pipe_labour_hours');await control.focus();control.value=String(value);await control.emit('input');await control.blur();await h.audit.calculate();control=h.control('pipe_labour_hours');assert.equal(h.audit.state.draft.rows[0].inputs.pipe_labour_hours,value);await control.focus();assert.equal(control.value,String(value));await control.blur();}
    control=h.control('pipe_labour_hours');control.value='-1';await control.emit('input');assert.match(h.api.inputProblem(),/invalid/);assert.equal(h.audit.state.draft.rows[0].inputs.pipe_labour_hours,.123456789012345);await control.parentNode.children[3].children[1].emit('click');await h.audit.calculate();assert.equal(h.api.inputProblem(),'');assert.equal(h.audit.state.draft.rows[0].inputs.pipe_labour_hours,null);assert.equal(h.control('pipe_labour_hours').value,'0.30');
  });
  ...
  await check('Library named allowances are captured by Save and reopen while later edits survive an in-flight receipt',async h=>{
    const metadata=allowanceDefinition(),draft=copy(metadata.defaults);draft.rows[0].inputs={T:'Library fixture',O:1,pipe_labour_hours:.3456789012345};draft.globals.register_allowance_hours=0;h.api.present(fixture(draft,{definition:metadata,result:allowanceResult(draft,metadata)}));h.audit.state.group='Pipes';h.audit.renderFields();
    ...
    assert.equal(capture.draft.globals.register_allowance_hours,0);assert.equal(capture.draft.rows[0].inputs.pipe_labour_hours,.3456789012345);assert.equal(h.audit.state.draft.rows[0].inputs.pipe_labour_hours,.4567890123456);assert.equal(h.api.hasUnsavedChanges(),true);assert.equal(h.api.isOpen(),true);
  ...
  await check('Library SETTINGS exposes contextual Waste and pipe band fields as saved globals',async h=>{
    ...
    ];const draft=copy(metadata.defaults);Object.assign(draft.globals,{register_allowance_hours:.25,pipe_labour_50_hours:.25,waste_cabletrays:0});
    ...
    const waste=h.control('waste_cabletrays',true),pipe=h.control('pipe_labour_50_hours',true),register=h.control('register_allowance_hours',true);assert.ok(waste&&pipe&&register);assert.equal(waste.parentNode.children[0].textContent,'Waste (%)');assert.equal(waste.title,'Applies to Cabletrays.');assert.equal(register.parentNode.children[0].textContent,'Register Allowance (hrs)');
  ...
  await check('Library editor separates Settings and edits the same routing and task-hour bands as the main estimator',async h=>{
    ...
    const settingsText=text(h.byId('library-editor-fields'));assert.match(settingsText,/SERVICE-TAB ROUTING.*Service Tab.*Service Types.*LAGGED PIPES.*UNLAGGED PIPES.*PRECALCULATED TASK-HOUR BANDS.*Board Task Hours/);assert.doesNotMatch(settingsText,/Separate service types with semicolons|Edit both thresholds and hours/);
    ...
    const remove=walk(h.byId('library-editor-fields')).find(node=>/^Remove Board Task Hours band/.test(node.getAttribute('aria-label')||''));assert.equal(remove.title,remove.getAttribute('aria-label'));assert.equal(remove.children[0].textContent,'🗑');assert.notEqual(remove.textContent,'Remove');await remove.emit('click');assert.equal(h.audit.state.draft.globals.labour_bands.board.length,2);
  ...
  await check('Opening the library item leaves the full project state and raw invalid inputs intact',async h=>{
    ...
    const projectQuantity=h.byId('penetration-item-quantity').querySelectorAll('[data-penetration-field]').find(el=>el.dataset.penetrationField==='O');projectQuantity.value='1e-';await projectQuantity.emit('input');
