...
(async()=>{
  ...
  await check('Conditional Firestopping groups follow Type and Service Type without changing inputs',async h=>{
    const metadata=definition();metadata.groups=['Penetration','Additional Allowances','Unlagged Pipes','Lagged Pipes','Plastic Pipes','Cables/Bundles','Cabletrays','Substrate','Bulkhead'];metadata.group_labels={Penetration:'DETAILS',Cabletrays:'CABLE TRAYS','Cables/Bundles':'BUNDLES','Additional Allowances':'OTHER'};metadata.group_visibility={
      'Unlagged Pipes':{column:'K',route_key:'Unlagged Pipes',values:['Unlagged Pipes']},'Lagged Pipes':{column:'K',route_key:'Lagged Pipes',values:['Lagged Pipes']},'Plastic Pipes':{column:'K',route_key:'Plastic Pipes',values:['Plastic Pipes']},
      'Cables/Bundles':{column:'K',route_key:'Cables/Bundles',values:['Cable Bundles','D1 Power Cables']},Cabletrays:{column:'K',route_key:'Cabletrays',values:['D1 Power Cables','D2 Comms Cables','Cable Trays']},Substrate:{any:[{column:'L',values:['Oversized']},{column:'K',route_key:'Substrate',values:['Blank Seal','Access Panel','Linear Joints','Movement Joints','Fire Dampers']}]},Bulkhead:{column:'J',values:['Bulkheads']}};
    const routes={'Unlagged Pipes':['Unlagged Pipes'],'Lagged Pipes':['Lagged Pipes'],'Plastic Pipes':['Plastic Pipes'],'Cables/Bundles':['Cable Bundles','D1 Power Cables'],Cabletrays:['D1 Power Cables','D2 Comms Cables','Cable Trays'],Substrate:['Blank Seal','Access Panel','Linear Joints','Movement Joints','Fire Dampers']};
    for(const draft of [metadata.defaults,metadata.schedule_defaults])draft.globals.service_routes=copy(routes);
    metadata.row_fields.push({column:'K',label:'Service Type',group:'Penetration',type:'select',format:'text',units:'',options:['Unlagged Pipes','Lagged Pipes','Plastic Pipes','Cable Bundles','D1 Power Cables','Blank Seal'],default:null});
    ...
    h.audit.state.definition=metadata;h.audit.state.draft.globals.service_routes=copy(routes);h.audit.state.schedule.draft.globals.service_routes=copy(routes);h.audit.state.draft.rows[0].inputs={J:'HVAC',K:'Unlagged Pipes',T:'Long source description'};h.audit.renderFields();
    assert.equal(h.byId('penetration-row-heading').textContent,'Current item · HVAC · Unlagged Pipes');
    assert.match(text(h.byId('penetration-input-groups')),/DETAILS.*Unlagged Pipes.*OTHER/);assert.equal(h.byId('penetration-input-groups').children.at(-1).textContent,'OTHER');assert.doesNotMatch(text(h.byId('penetration-input-groups')),/Plastic Pipes|Cables\/Bundles|CABLE TRAYS|Bulkhead/);
    const service=h.control('K');service.value='Plastic Pipes';await service.emit('change');assert.match(text(h.byId('penetration-input-groups')),/Plastic Pipes/);assert.doesNotMatch(text(h.byId('penetration-input-groups')),/Unlagged Pipes/);
    h.audit.state.group='Plastic Pipes';h.audit.renderFields();assert.ok(h.control('AL'));assert.ok(h.control('AN'));assert.equal(h.control('AN').parentNode.children[0].textContent,'Collar Multiplier');assert.equal(h.control('AM'),undefined);
    h.audit.state.group='Penetration';h.audit.renderFields();const lagged=h.control('K');lagged.value='Lagged Pipes';await lagged.emit('change');assert.match(text(h.byId('penetration-input-groups')),/Lagged Pipes/);assert.doesNotMatch(text(h.byId('penetration-input-groups')),/Unlagged Pipes/);
    h.audit.state.group='Lagged Pipes';h.audit.renderFields();assert.ok(h.control('AL'));assert.ok(h.control('AM'));assert.ok(h.control('AN'));assert.equal(h.control('AN').parentNode.children[0].textContent,'Wrap Multiplier');
  ...
  await check('SETTINGS synchronizes service routing, editable band units and hours, and Waste across composer and schedule',async h=>{
    ...
    metadata.global_fields=[
      ...
      {column:'waste_unlagged_pipes',label:'Waste',group:'SETTINGS',type:'number',format:'percent',units:'%',options:[],default:0,min:0,step:1,help:'Applies to Unlagged Pipes.'},
      {column:'waste_lagged_pipes',label:'Waste',group:'SETTINGS',type:'number',format:'percent',units:'%',options:[],default:0,min:0,step:1,help:'Applies to Lagged Pipes.'},
      {column:'waste_plastic_pipes',label:'Waste',group:'SETTINGS',type:'number',format:'percent',units:'%',options:[],default:0,min:0,step:1,help:'Applies to Plastic Pipes.'},
      {column:'waste_bundles',label:'Waste',group:'SETTINGS',type:'number',format:'percent',units:'%',options:[],default:0,min:0,step:1,help:'Applies to Bundles.'}
    ];
    metadata.settings={service_routes:[{key:'Unlagged Pipes',label:'Unlagged Pipes'},{key:'Lagged Pipes',label:'Lagged Pipes'}],labour_bands:[
      {key:'pipe',label:'Pipe Labour',basis:'Collar diameter',units:'mm',overflow:'manual'},
      {key:'board',label:'Board Task Hours',basis:'Board area',units:'m²',overflow:'last'},
      {key:'mastic',label:'Mastic Task Hours',basis:'Mastic volume',units:'L',overflow:'last'},
      {key:'framing',label:'Framing Task Hours',basis:'Framing length',units:'m',overflow:'last'},
      {key:'wrap',label:'Wrap Task Hours',basis:'Wrap area',units:'m²',overflow:'last'}],structured_global_keys:['service_routes','labour_bands']};
    const routes={'Unlagged Pipes':['Unlagged Pipes'],'Lagged Pipes':['Lagged Pipes']},bands={pipe:[{maximum:50,hours:.25},{maximum:100,hours:.3}],board:[{maximum:.15,hours:.5}],mastic:[{maximum:1,hours:.2}],framing:[{maximum:1,hours:.3}],wrap:[{maximum:.1,hours:.25}]};
    for(const draft of [metadata.defaults,metadata.schedule_defaults])Object.assign(draft.globals,{register_allowance_hours:.25,waste_unlagged_pipes:0,waste_lagged_pipes:0,waste_plastic_pipes:0,waste_bundles:0,service_routes:copy(routes),labour_bands:copy(bands)});
    ...
    const settingsRoot=h.byId('penetration-row-fields');
    assert.match(text(settingsRoot),/Setting.*Applies to.*Value.*Labour allowances.*Every Firestopping item.*Material waste.*SERVICE-TAB ROUTING.*PRECALCULATED TASK-HOUR BANDS.*Board Task Hours.*Wrap Task Hours/);
    assert.doesNotMatch(text(settingsRoot),/Separate service types with semicolons|Edit both thresholds and hours/);
    const settingNodes=node=>[node,...node.children.flatMap(settingNodes)],settingsHeadings=settingNodes(settingsRoot).filter(node=>node.className==='penetration-settings-subheading');assert.deepEqual(settingsHeadings.map(node=>node.textContent),['SERVICE-TAB ROUTING','PRECALCULATED TASK-HOUR BANDS']);
    const settingsCss=fs.readFileSync('static/penetration.css','utf8');assert.match(settingsCss,/\.penetration-settings-subheading\{[^}]*background:#fff0e7[^}]*text-transform:uppercase/);assert.match(settingsCss,/\.penetration-settings-table\{[^}]*min-width:0[^}]*table-layout:fixed/);
    const waste=h.control('waste_lagged_pipes',null),register=h.control('register_allowance_hours',null);assert.ok(waste&&register);assert.equal(waste.parentNode.children[0].textContent,'Waste (%)');assert.match(waste.title,/Lagged Pipes/);assert.equal(register.parentNode.children[0].textContent,'Register Allowance (hrs)');
    ...
    const descendants=node=>[node,...node.children.flatMap(descendants)],add=descendants(root).find(node=>node.tagName==='button'&&node.textContent==='+ Add band');await add.emit('click');assert.equal(h.audit.state.schedule.draft.globals.labour_bands.pipe.length,3);assert.equal(h.audit.state.draft.globals.labour_bands.pipe.length,3);
    const remove=descendants(h.byId('penetration-row-fields')).filter(node=>node.tagName==='button'&&/^Remove Pipe Labour band/.test(node.getAttribute('aria-label')||'')).at(-1);assert.equal(remove.title,remove.getAttribute('aria-label'));assert.equal(remove.children[0].textContent,'🗑');assert.notEqual(remove.textContent,'Remove');await remove.emit('click');assert.equal(h.audit.state.schedule.draft.globals.labour_bands.pipe.length,2);assert.equal(h.audit.state.draft.globals.labour_bands.pipe.length,2);
