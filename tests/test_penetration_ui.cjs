...
(async()=>{
  ...
  await check('SETTINGS synchronizes service routing, editable band units and hours, and Waste across composer and schedule',async h=>{
    ...
    for(const draft of [metadata.defaults,metadata.schedule_defaults])Object.assign(draft.globals,{register_allowance_hours:.25,waste_pipes:0,service_routes:copy(routes),labour_bands:copy(bands)});
    ...
    const waste=h.control('waste_pipes',null),register=h.control('register_allowance_hours',null);assert.ok(waste&&register);assert.equal(waste.parentNode.children[0].textContent,'Waste (%)');assert.match(waste.title,/Cables\/Bundles/);assert.equal(register.parentNode.children[0].textContent,'Register Allowance (hrs)');
    ...
    const root=h.byId('penetration-row-fields'),routeEditors=root.querySelectorAll('[data-penetration-service-route]'),bandEditors=root.querySelectorAll('[data-penetration-band]');assert.equal(routeEditors.length,2);assert.equal(bandEditors.length,12);
    ...
    const remove=descendants(h.byId('penetration-row-fields')).filter(node=>node.tagName==='button'&&/^Remove Pipe Labour band/.test(node.getAttribute('aria-label')||'')).at(-1);await remove.emit('click');assert.equal(h.audit.state.schedule.draft.globals.labour_bands.pipe.length,2);assert.equal(h.audit.state.draft.globals.labour_bands.pipe.length,2);
