...
(async()=>{
  ...
  await check('SETTINGS synchronizes service routing, editable band units and hours, and Waste across composer and schedule',async h=>{
    ...
    assert.match(text(h.byId('penetration-row-fields')),/Setting.*Applies to.*Value.*Labour allowances.*Every Firestopping item.*Material waste.*Service-tab routing.*Precalculated task-hour bands.*Board Task Hours.*Wrap Task Hours/);
