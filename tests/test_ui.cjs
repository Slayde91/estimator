...
(async()=>{
  ...
  // Supplier edits update the shared sell price while legacy per-use values stay read-only.
  ...
  assert.equal(audit.state.catalog.rate_groups.primers[0].id,'primer');assert.equal(audit.state.catalog.rate_groups.topcoats[0].price,180.123456);
  ...
  assert.match(html,/id="link-project-folder"[^>]*link-action-button/);assert.match(html,/id="penetration-pdf"[\s\S]*?<span class="download-arrow">⇩<\/span>/);
  ...
  assert.match(stylesCss,/\.button\.recalculate-button\{[^}]*background:#fff[^}]*box-shadow:none/);assert.doesNotMatch(stylesCss,/\.button\.recalculate-button\{[^}]*#257a45/);assert.match(stylesCss,/\.button\.link-action-button\{[^}]*background:#fff[^}]*box-shadow:none/);
  ...
  assert.doesNotMatch(markup,/id="show-rate-overrides"|Show rate overrides|Sell rate override/);
  assert.doesNotMatch(source,/show-rate-overrides|Sell rate override/);
  assert.match(actionCss,/\.pricing-table td \{ vertical-align: top; \}/);
  ...
  for(const id of ['penetration-pdf','penetration-item-pdf'])assert.match(markup,new RegExp(`id="${id}"[^>]*class="button pdf-button icon-only schedule-download-button"[^>]*aria-label="Download PDF Schedule"[^>]*title="Download PDF Schedule"[\\s\\S]*?<span class="download-arrow">⇩<\\/span>`));
  ...
  assert.match(markup,/id="calculator-template"[^>]*class="[^"]*excel-button[^"]*"/);
  for(const id of ['calculator-reset','calculator-recalculate'])assert.match(markup,new RegExp(`id="${id}"[^>]*class="[^"]*secondary[^"]*"`));
  assert.doesNotMatch(markup,/id="calculator-reset"[^>]*pricing-reset-button/);
  ...
  assert.match(actionCss,/\.pricing-discard-button,\.pricing-reset-button\{[^}]*background:#fff[^}]*border:1px solid #dcd3d7/);
  assert.match(actionCss,/\.button\.refresh-button\{[^}]*background:#fff[^}]*box-shadow:none/);
