...
(async()=>{
  ...
  assert.match(stylesCss,/\.button\.recalculate-button\{[^}]*background:#257a45/);assert.match(stylesCss,/\.button\.link-action-button\{[^}]*background:#d97706/);
  ...
  assert.match(markup,/id="save-pricing"[^>]*class="button primary icon-only"[^>]*aria-label="Save pricing"[^>]*title="Save pricing"/);
