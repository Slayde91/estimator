...
(async()=>{
  ...
  assert.match(markup,/<details class="card expandable-breakdown penetration-schedule"[\s\S]*?<div class="penetration-row-tools penetration-schedule-downloads"><button id="penetration-excel"[\s\S]*?<\/details>/);
  ...
  for(const id of ['penetration-item-pdf','penetration-estimator-schedule-body','penetration-estimator-schedule-recalculate','penetration-estimator-add','penetration-estimator-undo'])assert.match(markup,new RegExp(`id="${id}"`));
